  ...
  try {
    ...
    if (percentFull < 20) {
      Activity.create({
        ...
        description: `${tank.fuelType} (${tank.title}) below 20% - ${Number(tank.currentQuantity ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} Ltrs remaining`,
        timestamp: new Date(),
        severity: "warning",
      ...
      notifyStation(fillingStationId, {
        ...
        body: `${tank.fuelType} tank ${tank.title} is below 20% - ${Number(tank.currentQuantity ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} Ltrs remaining`,
        severity: "warning",
        targetRole: "manager",
        expiresInDays: 1,
      });
