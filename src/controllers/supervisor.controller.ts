...
export const exportReport = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    switch (reportType) {
      case "sales":
        const salesShifts = await Shift.find({
          fillingStation: stationId,
          shiftDate: { $gte: start, $lte: end },
          status: "Completed",
        })
      ...
      case "cash-reconciliation":
        data = await CashReconciliation.find({
          fillingStation: stationId,
          shiftDate: { $gte: start, $lte: end },
        })
          .populate("attendant", "firstName lastName")
      ...
      case "fuel-inventory":
        const tanks = await Tank.findOne({ fillingStation: stationId }).lean();
      ...
      case "staff-performance":
        data = await Staff.find({
          ...
          .select("firstName lastName role shiftType onDuty amount")
          .lean();
        break;

      case "lubricant-inventory":
        data = await LubricantSale.find({
          fillingStation: stationId,
