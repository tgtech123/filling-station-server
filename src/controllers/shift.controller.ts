...
export const endShift = async (req: AuthenticatedRequest, res: Response) => {
  try {
      ...
      .then(async (tankDoc) => {
        ...
        if (percentFilled < 20) {
          Activity.create({
            ...
            description: `${updatedTank.fuelType} tank is below 20% - ${Number(updatedTank.currentQuantity ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}L remaining`,
            timestamp: new Date(),
            severity: "warning",
            expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
    ...
    Staff.findById(attendantId).lean()
      .then((attendant: any) => {
        ...
        return Activity.create({
          ...
          description: `${name} - ${Number(shift.litresSold ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} Ltrs sold`,
          timestamp: new Date(),
          severity: null,
          expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000),
        });
    ...
    Notification.create({
      ...
      body: `Shift ended on ${shift.pumpTitle}. ${Number(shift.litresSold ?? 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} Ltrs sold, revenue: ₦${(shift.totalAmount ?? 0).toLocaleString()}`,
      severity: "info",
      timestamp: new Date(),
